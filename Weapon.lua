...
function Weapon:init(name,team,xoffset,yoffset,card)
    ...
    for i=1,self.weaponCount do
        if name['weapon' .. tostring(i)] then
            ...
            self.Weapons[i] = Weapon2(weaponImages[name['weapon'..tostring(i)]],i,team,xoffset,yoffset,card)
        else 
            self.Weapons[i] = Weapon2(weaponImages[name['weapon1']],i,team,xoffset,yoffset,card)
        end
    end
end
